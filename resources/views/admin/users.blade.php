...
<head>
    ...
    <title>Kelola User - SPPG Kokrosono</title>
    <!-- CDN Tailwind CSS -->
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 font-sans antialiased flex h-screen overflow-hidden">

    <!-- MODAL FORM (Tambah & Edit User MySQL) -->
    <div id="user-modal" class="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center hidden z-50 p-4 transition-all duration-300">
        <div class="bg-white rounded-2xl border border-gray-100 w-full max-w-md p-6 shadow-xl transform scale-95 transition-all duration-300" id="modal-container">
            <div class="flex justify-between items-center mb-4">
                <h3 id="modal-title" class="text-lg font-bold text-gray-900">Tambah User Baru</h3>
                <button onclick="closeModal()" class="text-gray-400 hover:text-gray-600">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>
            
            <!-- Form Action yang dinamis diarahkan menggunakan JS (menyembah route store atau update) -->
            <form id="modal-form" method="POST" action="{{ route('admin.users.store') }}" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="username" id="input-username" required placeholder="Masukkan username" class="w-full border border-gray-200 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-[#071E48] focus:border-[#071E48] outline-none transition">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Role / Jabatan *</label>
                    <select name="role" id="input-role" required class="w-full border border-gray-200 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-[#071E48] focus:border-[#071E48] outline-none transition cursor-pointer">
                        <option value="Super Admin">Super Admin</option>
                        <option value="Staff Gizi">Staff Gizi</option>
                        <option value="Petugas Sekolah">Petugas Sekolah</option>
                    </select>
                </div>
                <div>
                    <label id="password-label" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Password *</label>
                    <input type="password" name="password" id="input-password" placeholder="Masukkan password" class="w-full border border-gray-200 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-[#071E48] focus:border-[#071E48] outline-none transition">
                    <p id="password-help" class="text-[10px] text-gray-400 mt-1 hidden">Kosongkan jika tidak ingin mengubah password.</p>
                </div>
    ...
    <div id="delete-modal" class="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center hidden z-50 p-4 transition-all duration-300">
        <div class="bg-white rounded-2xl border border-gray-100 w-full max-w-sm p-6 shadow-xl transform scale-95 transition-all duration-300" id="delete-modal-container">
            <div class="flex flex-col items-center text-center">
                ...
                <p class="text-sm text-gray-500 mb-6">Tindakan ini tidak dapat dibatalkan. Pengguna <strong id="delete-target-name" class="text-gray-950"></strong> akan dihapus permanen dari MySQL.</p>
            ...
            <div>
                ...
                <div class="space-y-1">
                    ...
                    <a href="#" class="flex items-center gap-3 px-6 py-3 text-sm font-semibold bg-gray-900 border-l-4 border-[#D4B16D] text-white transition">
                        <svg class="w-5 h-5 text-[#D4B16D]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                        Users
                    </a>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="bg-white shadow-xs h-16 flex items-center justify-between px-6 border-b border-gray-200 shrink-0">
            <h1 class="text-lg font-bold text-gray-900">Kelola Pengguna / Users</h1>
        ...
        <main class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="relative w-full md:max-w-xs">
                    ...
                    <input type="text" id="search-input" oninput="searchUser()" placeholder="Cari username..." class="w-full bg-white border border-gray-200 rounded-xl py-2.5 pl-10 pr-4 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition shadow-xs">
                </div>
            ...
            <!-- Tabel Data Pengguna dari MySQL -->
            <div class="bg-white rounded-xl shadow-xs border border-gray-200 overflow-hidden">
                <div class="p-4 border-b border-gray-200 bg-gray-50 font-bold text-gray-700 text-sm flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <span>Daftar Staff & Petugas SPPG Kokrosono</span>
                    </div>
                    <span class="text-xs bg-gray-200 text-gray-700 px-2.5 py-1 rounded-md" id="user-count">Total: {{ $allUsers->count() }}</span>
                </div>
                    ...
                    <table class="w-full text-left border-collapse" id="users-table">
                        <thead>
                            <tr class="bg-gray-100 text-gray-600 text-xs uppercase tracking-wider border-b border-gray-200">
                                ...
                                <th class="p-4 font-semibold">Role / Jabatan</th>
                        ...
                        <tbody>
                            @forelse($allUsers as $index => $user)
                                <tr class="user-row border-b border-gray-200 hover:bg-gray-50/50 text-sm text-gray-700 transition">
                                    <td class="p-4 font-semibold text-gray-500">{{ $index + 1 }}</td>
                                    <td class="p-4 font-bold text-gray-900 user-name-text">{{ $user->name }}</td>
                                    <td class="p-4">
                                        <span class="px-2.5 py-1 rounded-md text-xs font-bold border 
                                            {{ $user->role == 'Super Admin' ? 'bg-red-50 text-red-700 border-red-200' : '' }}
                                            {{ $user->role == 'Staff Gizi' ? 'bg-green-50 text-green-700 border-green-200' : '' }}
                                            {{ $user->role == 'Petugas Sekolah' ? 'bg-blue-50 text-blue-700 border-blue-200' : '' }}
                                        ">
                                            {{ $user->role }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="p-4 text-right">
                                        <div class="inline-flex gap-2">
                                            ...
                                            <button onclick="openEditModal('{{ $user->id }}', '{{ $user->name }}', '{{ $user->role }}')" class="p-1.5 bg-yellow-50 text-yellow-600 hover:bg-yellow-100 rounded-lg border border-yellow-200 transition" title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </button>
                                            
                                            <!-- Tombol Hapus (Kirim Data ke modal konfirmasi) -->
                                            <button onclick="openDeleteModal('{{ $user->id }}', '{{ $user->name }}')" class="p-1.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg border border-red-200 transition" title="Hapus">
                                ...
                                <tr>
                                    <td colspan="5" class="p-12 text-center text-gray-400">
                                        <div class="flex flex-col items-center justify-center space-y-2">
                                            ...
                                            <p class="font-medium text-sm text-gray-500">Tidak ada user terdaftar di MySQL.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
    ...
    <script>
        ...
        function openAddModal() {
            ...
            document.getElementById('modal-form').action = "{{ route('admin.users.store') }}";
        ...
        function openEditModal(id, username, role) {
            document.getElementById('modal-title').textContent = "Edit Data Pengguna";
            document.getElementById('input-username').value = username;
            document.getElementById('input-role').value = role;
            
            // Atur URL Action dinamis untuk Update data berdasarkan ID User
            document.getElementById('modal-form').action = "/admin/users/" + id;
            document.getElementById('method-container').innerHTML = '@csrf @method("PUT")';

            // Password opsional saat melakukan edit
        ...
        function openDeleteModal(id, username) {
            document.getElementById('delete-target-name').textContent = username;
            // Atur URL Action dinamis untuk menghapus data berdasarkan ID User
        ...
        // Pencarian Real-Time Sisi Client (Sangat cepat & menghemat kueri database MySQL)
        function searchUser() {
            const query = document.getElementById('search-input').value.toLowerCase();
            const rows = document.querySelectorAll('.user-row');
            
            rows.forEach(row => {
                const usernameText = row.querySelector('.user-name-text').textContent.toLowerCase();
