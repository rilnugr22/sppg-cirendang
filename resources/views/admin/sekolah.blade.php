...
<head>
    ...
    <title>Kelola Sekolah - SPPG Kokrosono</title>
    <!-- CDN Tailwind CSS -->
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 font-sans antialiased flex h-screen overflow-hidden">

    <!-- MODAL FORM (Tambah & Edit Sekolah MySQL) -->
    <div id="sekolah-modal" class="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center hidden z-50 p-4 transition-all duration-300">
        <div class="bg-white rounded-2xl border border-gray-100 w-full max-w-md p-6 shadow-xl transform scale-95 transition-all duration-300" id="modal-container">
            <div class="flex justify-between items-center mb-4">
                <h3 id="modal-title" class="text-lg font-bold text-gray-900">Tambah Sekolah Baru</h3>
                <button onclick="closeModal()" class="text-gray-400 hover:text-gray-600">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>
            
            <!-- Form Action yang dinamis diarahkan menggunakan JS (menyasar route store atau update) -->
            <form id="modal-form" method="POST" action="{{ route('admin.sekolah.store') }}" class="space-y-4">
                @csrf
                <div id="method-container"></div> <!-- Berisi input @method('PUT') secara dinamis saat edit -->
                
                <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Nama Sekolah *</label>
                    <input type="text" name="nama_sekolah" id="input-nama" required placeholder="Masukkan nama sekolah (misal: SD Negeri 01)" class="w-full border border-gray-200 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-[#071E48] focus:border-[#071E48] outline-none transition">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Alamat Sekolah *</label>
                    <textarea name="alamat" id="input-alamat" required rows="3" placeholder="Masukkan alamat lengkap sekolah..." class="w-full border border-gray-200 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-[#071E48] focus:border-[#071E48] outline-none transition resize-none"></textarea>
                </div>
    ...
    <div id="delete-modal" class="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center hidden z-50 p-4 transition-all duration-300">
        <div class="bg-white rounded-2xl border border-gray-100 w-full max-w-sm p-6 shadow-xl transform scale-95 transition-all duration-300" id="delete-modal-container">
            <div class="flex flex-col items-center text-center">
                ...
                <h3 class="text-lg font-bold text-gray-900 mb-1">Hapus Sekolah?</h3>
                <p class="text-sm text-gray-500 mb-6">Tindakan ini tidak dapat dibatalkan. Data sekolah <strong id="delete-target-name" class="text-gray-950"></strong> akan dihapus permanen dari MySQL.</p>
            ...
            <div>
                ...
                <div class="space-y-1">
                    ...
                    <a href="#" class="flex items-center gap-3 px-6 py-3 text-sm font-semibold bg-gray-900 border-l-4 border-[#D4B16D] text-white transition">
                        <svg class="w-5 h-5 text-[#D4B16D]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
                        Sekolah
                    </a>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="bg-white shadow-xs h-16 flex items-center justify-between px-6 border-b border-gray-200 shrink-0">
            <h1 class="text-lg font-bold text-gray-900">Kelola Sekolah Penerima MBG</h1>
        ...
        <main class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="relative w-full md:max-w-xs">
                    ...
                    <input type="text" id="search-input" oninput="searchSekolah()" placeholder="Cari nama sekolah..." class="w-full bg-white border border-gray-200 rounded-xl py-2.5 pl-10 pr-4 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition shadow-xs">
                </div>
            ...
            <div class="bg-white rounded-xl shadow-xs border border-gray-200 overflow-hidden">
                <div class="p-4 border-b border-gray-200 bg-gray-50 font-bold text-gray-700 text-sm flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <span>Daftar Wilayah Distribusi Sekolah MBG</span>
                    </div>
                    <span class="text-xs bg-gray-200 text-gray-700 px-2.5 py-1 rounded-md" id="sekolah-count">Total: {{ $allSekolah->count() }}</span>
                </div>

                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse" id="sekolah-table">
                        <thead>
                            <tr class="bg-gray-100 text-gray-600 text-xs uppercase tracking-wider border-b border-gray-200">
                                ...
                                <th class="p-4 font-semibold">Alamat Lengkap</th>
                                <th class="p-4 font-semibold text-right">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($allSekolah as $index => $sekolah)
                                <tr class="sekolah-row border-b border-gray-200 hover:bg-gray-50/50 text-sm text-gray-700 transition">
                                    <td class="p-4 font-semibold text-gray-500">{{ $index + 1 }}</td>
                                    <td class="p-4 font-bold text-gray-900 sekolah-name-text">{{ $sekolah->nama_sekolah }}</td>
                                    <td class="p-4 text-gray-600 max-w-sm truncate" title="{{ $sekolah->alamat }}">
                                        {{ $sekolah->alamat }}
                                    </td>
                                    <td class="p-4 text-right">
                                        <div class="inline-flex gap-2">
                                            <!-- Tombol Edit (Kirim Data ke modal JS) -->
                                            <button onclick="openEditModal('{{ $sekolah->id }}', '{{ addslashes($sekolah->nama_sekolah) }}', '{{ addslashes($sekolah->alamat) }}')" class="p-1.5 bg-yellow-50 text-yellow-600 hover:bg-yellow-100 rounded-lg border border-yellow-200 transition" title="Edit">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </button>
                                            
                                            <!-- Tombol Hapus (Kirim Data ke modal konfirmasi) -->
                                            <button onclick="openDeleteModal('{{ $sekolah->id }}', '{{ addslashes($sekolah->nama_sekolah) }}')" class="p-1.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg border border-red-200 transition" title="Hapus">
                                ...
                                <tr>
                                    <td colspan="4" class="p-12 text-center text-gray-400">
                                        <div class="flex flex-col items-center justify-center space-y-2">
                                            ...
                                            <p class="font-medium text-sm text-gray-500">Tidak ada sekolah terdaftar di MySQL.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
    ...
    <script>
        // Buka Modal Tambah Sekolah Baru
        function openAddModal() {
            document.getElementById('modal-title').textContent = "Tambah Sekolah Baru";
            document.getElementById('modal-form').reset();
            document.getElementById('modal-form').action = "{{ route('admin.sekolah.store') }}";
        ...
        // Buka Modal Edit Sekolah
        function openEditModal(id, namaSekolah, alamat) {
            document.getElementById('modal-title').textContent = "Edit Data Sekolah";
            document.getElementById('input-nama').value = namaSekolah;
            document.getElementById('input-alamat').value = alamat;
            
            // Atur URL Action dinamis untuk Update data berdasarkan ID Sekolah
            document.getElementById('modal-form').action = "/admin/sekolah/" + id;
        ...
        function openDeleteModal(id, namaSekolah) {
            document.getElementById('delete-target-name').textContent = namaSekolah;
            // Atur URL Action dinamis untuk menghapus data berdasarkan ID Sekolah
            document.getElementById('delete-form').action = "/admin/sekolah/" + id;
        ...
        function searchSekolah() {
            ...
            rows.forEach(row => {
                const nameText = row.querySelector('.sekolah-name-text').textContent.toLowerCase();
